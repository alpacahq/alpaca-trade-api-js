...
function Alpaca(config = {}) {
  ...
  let envs = {}
  try {
    envs = process.env //TODO: find a better way to do this
  } catch (e) {
    envs = {};
  }
  this.configuration = {
    baseUrl: config.baseUrl || envs.APCA_API_BASE_URL
      || (config.paper ? 'https://paper-api.alpaca.markets' : 'https://api.alpaca.markets'),
    dataBaseUrl: config.dataBaseUrl || envs.APCA_DATA_BASE_URL || envs.DATA_PROXY_WS || 'https://data.alpaca.markets',
    polygonBaseUrl: config.polygonBaseUrl || envs.POLYGON_API_BASE_URL || 'https://api.polygon.io',
    keyId: config.keyId || envs.APCA_API_KEY_ID || '',
    secretKey: config.secretKey || envs.APCA_API_SECRET_KEY || '',
    apiVersion: config.apiVersion || envs.APCA_API_VERSION || 'v2',
    oauth: config.oauth || envs.APCA_API_OAUTH || '',
