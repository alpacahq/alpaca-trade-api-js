...
function Alpaca(config = {}) {
  this.configuration = {
    baseUrl: config.baseUrl || process.env.APCA_API_BASE_URL || 'https://api.alpaca.markets/v1',
    dataBaseUrl: config.dataBaseUrl || process.env.APCA_DATA_BASE_URL || 'https://data.alpaca.markets/v1',
    polygonBaseUrl: config.polygonBaseUrl || process.env.POLYGON_API_BASE_URL || 'https://api.polygon.io/v1',
    keyId: config.keyId || process.env.APCA_API_KEY_ID || '',
    secretKey: config.secretKey || process.env.APCA_API_SECRET_KEY || '',
  }
  this.websockets.client.setConfiguration(
...
Alpaca.prototype.getOpenClosePrices = polygon.openClosePrices
