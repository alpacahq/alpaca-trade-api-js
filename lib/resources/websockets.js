...
class AlpacaStreamClient extends events.EventEmitter {
  ...
  onTrades(fn) {
    this.on(EVENT.STOCK_TRADES, function (data) { fn(data) })
  }

  onQuotes(fn) {
    this.on(EVENT.STOCK_QUOTES, function (data) { fn(data) })
  }

  onAggSec(fn) {
    this.on(EVENT.STOCK_AGG_SEC, function (data) { fn(data) })
  }

  onAggMin(fn) {
    this.on(EVENT.STOCK_AGG_MIN, function (data) { fn(data) })
  }
    ...
    switch (subject) {
      ...
      default:
        if (message.stream.startsWith('T.')) {
          this.emit(EVENT.STOCK_TRADES, message.data)
        } else if (message.stream.startsWith('Q.')) {
          this.emit(EVENT.STOCK_QUOTES, subject, entity.Quote(message.data))
        } else if (message.stream.startsWith('AM.')) {
          this.emit(EVENT.STOCK_AGG_MIN, subject, entity.MinuteBar(message.data))
        } else if (message.stream.startsWith('A.')) {
          this.emit(EVENT.STOCK_AGG_SEC, message.data)
        } else {
