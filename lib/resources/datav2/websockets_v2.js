...
const EVENT = {
  ...
  TRADING_STATUSES: "trading_status",
};
...
class AlpacaStreamV2Client extends events.EventEmitter {
  ...
  subscribe(trades, quotes, bars, dailyBars, statuses) {
    const subMsg = {
      action: "subscribe",
      trades: trades,
      quotes: quotes,
  ...
  subscribeAll() {
    if (
      ...
      this.session.subscriptions.dailyBars.length > 0 ||
      this.session.subscriptions.statuses.length > 0
    ) {
      const msg = {
        action: "subscribe",
  ...
  unsubscribeFromStatuses(statuses) {
    this.session.subscriptions.statuses =
      this.session.subscriptions.statuses.filter((status) => 
        !statuses.includes(status)
      );
    this.unsubscribe([], [], [], [], statuses);
  }

  unsubscribe(trades, quotes, bars, dailyBars, statuses) {
    const unsubMsg = {
      action: "unsubscribe",
      trades: trades,
      quotes: quotes,
