...
async function* getDataV2(endpoint, path, options = {}, config) {
  ...
  while (true) {
    ...
    const resp = await dataHttpRequest(`${path}`, params, config);
...
async function* getCryptoTrades(
  ...
  const cryptoTrades = getDataV2(
    TYPE.TRADES,
    `/v1alpha1/crypto/${symbol}/trades`,
    { start: options.start, end: options.end, limit: options.limit },
    config
  );
  for await (let t of cryptoTrades) {
    yield entityv2.AlpacaCryptoTrade({ S: symbol, ...t.trades });
  }
}
...
async function* getCryptoQuotes(
  ...
  const cryptoQuotes = getDataV2(
    TYPE.QUOTES,
    `/v1alpha1/crypto/${symbol}/quotes`,
    options,
    config
  );
  for await (const q of cryptoQuotes) {
    yield entityv2.AlpacaCryptoQuote({ S: symbol, ...q.quote });
  }
}
...
async function* getCryptoBars(
  ...
  if (options.exchange == null || options.timeframe == null) {
    throw new Error("missing required field(s)");
  }
  const cryptoBars = getDataV2(
    TYPE.BARS,
    `/v1alpha1/crypto/${symbol}/bars`,
    options,
    config
  );
  for await (const b of cryptoBars) {
    yield entityv2.AlpacaCryptoBar({ S: symbol, ...b.bars });
  }
}
...
async function getLatestCryptoTrade(
  ...
  const resp = await dataHttpRequest(
    `/v1alpha1/crypto/${symbol}/trades/latest`,
    options,
    config
  );
...
async function getLatestCryptoQuote(
  ...
  const resp = await dataHttpRequest(
    `/v1alpha1/crypto/${symbol}/quotes/latest`,
    options,
    config
  );
  console.log(resp.data);
...
async function getLatestCryptoXBBO(
  ...
  const resp = await dataHttpRequest(
    `/v1alpha1/crypto/${symbol}/xbbo/latest`,
    options,
    config
  );
}
