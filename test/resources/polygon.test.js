...
import mockPolygon from "../support/mock-server";
...
describe("polygon methods", () => {
  ...
  it("can get endpoints for a symbol", () => {
    return expect(alpaca.getSymbol("AAPL")).to.eventually.have.property(
      "endpoints"
    );
  });

  it("can get a company", () => {
    return expect(alpaca.getCompany("AAPL")).to.eventually.have.property(
      "name"
    );
  });

  it("can get analysts", () => {
    return expect(alpaca.getAnalysts("AAPL")).to.eventually.have.property(
      "strongBuy"
    );
  });
  ...
  it("can get historic aggregates v2", () => {
    return expect(
      alpaca.getHistoricAggregatesV2(
        "AAPL",
        1,
        "day",
        "2018-02-01",
  ...
  it("can get the symbol type map", () => {
    return expect(alpaca.getSymbolTypeMap()).to.eventually.have.property("cs");
  });
